...
// ── voice pool (all guaranteed available with espeak-ng on Ubuntu) ────────────
const VOICES = [
  "en",
...
const SENTENCES = {
  1: [
    "Yes", "Okay teacher", "I see", "Got it", "Mm-hmm", "Sure", "Okay",
  ],
  2: [
...
// ── seeded RNG (xorshift32, deterministic per student) ────────────────────────
...
// ── assign noise level from mode + distribution ───────────────────────────────
function assignLevel(globalIndex, mode, dist) {
  ...
  // mix: distribute by cumulative percentage
  const total = dist.reduce((a, b) => a + b, 0);
  const slot  = globalIndex % total;
  let cumulative = 0;
...
// ── generate one student's audio WAV ─────────────────────────────────────────
function generateStudent(globalIndex, slot, batch, level, tmpDir, audioDir) {
  ...
  if (level === 0) {
    run(
      ...
      `-af "bandpass=f=150:w=200:width_type=h" -ar 48000 -ac 2 "${outFile}"`
    );
    return outFile;
  }
  ...
  // Generate speech segments (one per slot)
  const speechFiles = [];
  for (let j = 0; j < cfg.segments; j++) {
    ...
    // Convert to 48 kHz stereo (espeak outputs mono at its own rate)
    run(`ffmpeg -y -i "${raw}" -ar 48000 -ac 2 "${conv}"`);
  ...
  for (let j = 0; j <= cfg.segments; j++) {
    ...
    run(`ffmpeg -y -f lavfi -i "anullsrc=r=48000:cl=stereo" -t ${dur.toFixed(2)} "${sil}"`);
    ordered.push(sil);
  ...
  // Mix background noise into the concat (duration=first → output matches speech length)
  run(
    `ffmpeg -y -i "${concat}" ` +
...
for (let i = start; i < end; i++) {
  ...
  const levelName = ["quiet", "whisper", "talking", "noisy"][level];
  const voice     = level > 0 ? VOICES[i % VOICES.length] : "—";
  const speed     = level > 0 ? 115 + (i % 8) * 5 : 0;
  console.log(`[generate] user ${i} (slot ${slot}): level=${levelName} voice=${voice} speed=${speed || "n/a"}`);
